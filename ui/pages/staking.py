...
class StakingPage(QWidget):
    def __init__(self, rpc: RpcClient, wallet_mgr: WalletManager, parent=None):
        ...
        self._stake_label = QLabel(_("Your Stake: —"))
        ...
        self._rewards_label = QLabel(_("Rewards Earned: —"))
        ...
        self._apy_label = QLabel(_("APY: 12.5%"))
        ...
        self._total_staked_label = QLabel(_("Total Staked: —"))
        ...
        action_layout.addWidget(self._make_label(_("Stake Amount (atomic units)")))
        self._stake_input = QLineEdit()
        self._stake_input.setPlaceholderText(_("e.g. 100000000000 = 10 AETH"))
        ...
        self._stake_btn = QPushButton(_("Stake"))
        ...
        self._unstake_btn = QPushButton(_("Unstake All"))
    ...
    def _refresh(self):
        if not self._wallet_mgr.has_wallet:
            self._stake_label.setText(_("Your Stake: — (no wallet)"))
            self._rewards_label.setText(_("Rewards Earned: —"))
            self._total_staked_label.setText(_("Total Staked: —"))
            return
        try:
            ...
            if isinstance(staked, (int, float)):
                self._stake_label.setText(_("Your Stake: {:.4f} AETH").format(staked / 10_000_000_000))
            else:
                self._stake_label.setText(_("Your Stake: {}").format(staked))

            if isinstance(rewards, (int, float)):
                self._rewards_label.setText(_("Rewards Earned: {:.4f} AETH").format(rewards / 10_000_000_000))
            else:
                self._rewards_label.setText(_("Rewards Earned: {}").format(rewards))

            if isinstance(total_staked, (int, float)):
                self._total_staked_label.setText(_("Total Staked: {:.4f} AETH").format(total_staked / 10_000_000_000))
            else:
                self._total_staked_label.setText(_("Total Staked: {}").format(total_staked))
    ...
    def _on_unstake(self):
        ...
        if not QMessageBox.question(
            self, _("Confirm Unstake"),
            _("Unstake all tokens?\n\nRewards will be credited to your wallet."),
        ...
        self._result.setStyleSheet(f"color: {TEXT_SECONDARY}; background: transparent; padding: 8px;")
        self._result.setText(_("Unstaking..."))

        try:
            addr = self._wallet_mgr.wallet_data.get("address", "")
            resp = self._rpc.unstake_tokens(addr)
            self._result.setStyleSheet(f"color: {SUCCESS}; background: transparent; padding: 8px;")
            self._result.setText(_("Unstaked! {}").format(resp))
            QTimer.singleShot(1000, self._refresh)
        except RpcError as e:
            self._result.setStyleSheet(f"color: {ERROR}; background: transparent; padding: 8px;")
            self._result.setText(_("Error: {}").format(e))
        finally:
